...
class VecTask():
    def __init__(self, task, rl_device, clip_observations=255.0, clip_actions=1.0):
        self.task = task
        ...
        self.obs_states_space = spaces.Box(np.ones(self.num_obs_states) * -np.Inf, np.ones(self.num_obs_states) * np.Inf)
...
class VecTaskPython(VecTask):
    ...
    def get_state(self):
        # return torch.clamp(self.task.obs_states_buf, 0, self.clip_obs).to(self.rl_device)
        return self.task.obs_states_buf.to(self.rl_device)
    ...
    def step(self, actions):
        ...
        return self.task.obs_states_buf.to(self.rl_device), self.task.rew_buf.to(self.rl_device), self.task.reset_buf.to(self.rl_device), self.task.extras
