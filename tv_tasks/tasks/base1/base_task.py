...
class BaseTask():
    ...
    def __init__(self, cfg, enable_camera_sensors=False):
        ...
        # (3, 224, 224)
        if cfg["env"]["obs_type"] in ["Vision"]:
            H, W, C = self.img_dim
            self.img_buf = torch.zeros(
                (self.num_envs, H, W, C), device=self.device, dtype=torch.float)
        if cfg["env"]["obs_type"] in ["DexRep_debug"]:
            H, W, C = self.img_dim
            self.depth_buf = torch.zeros(
                (self.num_envs, H, W), device=self.device, dtype=torch.float)
            self.pc_buf = torch.zeros(
                (self.num_envs, 224*224, 3), device=self.device, dtype=torch.float)
        ...
        if self.headless == False:
            ...
            if sim_params.up_axis == gymapi.UP_AXIS_Z:
                cam_pos = gymapi.Vec3(35.0, 35.0, 3.0) # cam_pos = gymapi.Vec3(20.0, 25.0, 3.0)
                cam_target = gymapi.Vec3(45.0, 45.0, 0.0)# cam_target = gymapi.Vec3(10.0, 15.0, 0.0)
            else:
                cam_pos = gymapi.Vec3(0.0, 0.0, 3.0)  # cam_pos = gymapi.Vec3(20.0, 25.0, 3.0)
                cam_target = gymapi.Vec3(10.0, 10.0, 0.0)  # cam_target = gymapi.Vec3(10.0, 15.0, 0.0)

            cam_pos = gymapi.Vec3(35.0, 35.0, 3.0)  # cam_pos = gymapi.Vec3(20.0, 25.0, 3.0)
            cam_target = gymapi.Vec3(0.0, 0.0, 0.0)  # cam_target = gymapi.Vec3(10.0, 15.0, 0.0)
